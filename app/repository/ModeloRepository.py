...
from app.model.ModeloModel import Modelo
from app.schema.ModeloSchema import ModeloCreate, PageResponse
...
class ModeloRepository:
    ...
    # CREATE
    @staticmethod
    async def create(create_form: ModeloCreate):
        nome_upcase = create_form.nome.upper()
        db.add(Modelo(nome=nome_upcase))
        await commit_rollback()

    # GET BY ID
    @staticmethod
    async def get_by_id(modelo_id: int):
        """ retrieve data by id """
        query = select(Modelo).where(Modelo.id == modelo_id)
    ...
    # DELETE
    @staticmethod
    async def delete(modelo_id: int):
        """ delete data by id """

        query = delete(Modelo).where(Modelo.id == modelo_id)
        await db.execute(query)
        await commit_rollback()
    ...
    async def get_all(
        ...
        query = select(from_obj=Modelo, columns="*")

        # select columns dynamically
        if columns is not None and columns != "all":
            # we need column format data like this --> [column(id),column(name),column(sex)...]

            query = select(from_obj=Modelo, columns=convert_columns(columns))
